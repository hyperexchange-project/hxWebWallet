<template>
  <div>
    <div class="hx-main-container hx-broadcast-tx-container">
      ...
      <div v-if="!broadcastTxDone">
        <el-form
          ...
          ref="broadcastTxForm"
          label-width="90pt"
          class="hx-broadcast-tx-inner-container"
        >
          <div>
            <el-input
              class="-input-address"
      ...
      <div v-if="broadcastTxDone" class="hx-broadcast-tx-done-container">
...
export default {
  ...
  methods: {
    ...
    toBroadcastTx(content) {
      try {
        console.log('to broadcast tx: ' + content);
        ...
        nodeClient.apiInstance.network_api().exec("broadcast_transaction_synchronous", [txJson]).then(
          data => {
            console.log(data); // show txid
            this.connected = true;
            if(data instanceof String) {
              this.broadcastTxForm.broadcastTxResult = 'txid: ' + data;
            } else {
              this.broadcastTxForm.broadcastTxResult = 'result: ' + JSON.stringify(data);
            }
          },
          this.showError
        );
...
.hx-broadcast-tx-container2 {
  .label-font {
    color: #a99eb4;
    font-size: 8pt;
  }
}
.hx-broadcast-tx-container,
.hx-broadcast-tx-done-container {
  min-width: 400px;
  min-height: 266pt;
  .-broadcast-tx-title {
    font-size: 20pt;
